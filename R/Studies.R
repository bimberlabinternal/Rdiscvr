...
ApplyMalariaMetadata <- function(seuratObj, errorIfUnknownIdsFound = TRUE, reApplyMetadata = TRUE) {
  ...
  if ('Challenge-1' %in% seuratObj$TimepointLabel) {
    seuratObj$TimepointLabel <- forcats::fct_relevel(seuratObj$TimepointLabel, 'Challenge-1', after = ifelse('Baseline-1' %in% seuratObj$TimepointLabel, yes = which(levels(seuratObj$TimepointLabel) == 'Baseline-1'), no = 0L))
  }
...
ApplyEC_Metadata <- function(seuratObj, errorIfUnknownIdsFound = TRUE, reApplyMetadata = TRUE) {
  ...
  metadata2 <- labkey.selectRows(
    ...
    viewName="",
    colSelect="Id,date,timePostSivChallenge/daysPostInfection,viralLoad",
    colSort="Id",
  ...
  metadata2$TimepointLabel <- case_when(
    metadata2$Timepoint == 'Baseline' ~ 'Baseline',
    metadata2$Timepoint == 'D0' ~ 'Baseline',
    metadata2$DPI %in% c(7,8,10) ~ 'DPI_7-10',
    metadata2$DPI %in% c(14,15) ~ 'DPI_14-15',
    metadata2$DPI %in% c(21,22,23) ~ 'DPI_21-23',
    metadata2$DPI %in% c(28,29,30) ~ 'DPI_28-30',
    metadata2$DPI > 200 ~ 'DPI>200',
    .default = 'UNKNOWN'
  )

  metadata2$TimepointLabel <- naturalsort::naturalfactor(metadata2$TimepointLabel)
