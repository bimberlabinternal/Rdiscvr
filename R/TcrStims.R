...
#' @param xFacetField2 An optional field used for faceting. If provided, it will be combined with xFacetField using facet_nested
...
#' @export
#' @import dplyr
GenerateTcrPlot <- function(dat, xFacetField = NA, xFacetField2 = NA, plotTitle = NULL, yFacetField = 'IsActiveLabel', patternField = 'IsShared', dropInactive = FALSE, labelUsingCounts = TRUE, groupLowFreq = FALSE) {
  if (dropInactive) {
    dat <- dat %>%
      filter(IsActive)
  }
  ...
  if (xFacetField != '.' || yFacetField != '.') {
    wrap_by <- function(xFacetField, yFacetField, xFacetField2) {
      if (xFacetField2 == '.') {
        ...
        return(ggh4x::facet_nested(vars(!!sym(yFacetField)), vars(!!sym(xFacetField), !!sym(xFacetField2)), scales = 'free', space = 'free_x'))
      }
    }

    PT <- PT + wrap_by(xFacetField, yFacetField, xFacetField2)
  }
...
.IdentifyActiveClonotypes <- function(seuratObj, chain = 'TRB', method = 'sPLS', maxRatioToCombine = 1.0, minOddsRatio = 0.5, minEDS = 2) {
  ...
  for (subjectId in sort(unique(seuratObj$SubjectId))) {
    ...
    P1 <- GenerateTcrPlot(dat, xFacetField = 'Tissue', xFacetField2 = 'SampleDate', dropInactive = FALSE, patternField = 'IsFiltered', plotTitle = paste0(subjectId, ": EDS > ", minEDS, ", Unfiltered"), groupLowFreq = TRUE)
